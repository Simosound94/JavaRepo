...
public class IndexMinPQ<Key extends Comparable<Key>> implements Iterable<Integer> {
    ...
    private int[] t2i;        // binary heap using 1-based indexing
    private int[] i2t;        // inverse of pq - qp[pq[i]] = pq[qp[i]] = i
    ...
    public IndexMinPQ(int maxN) {
        ...
        t2i   = new int[maxN + 1];
        i2t   = new int[maxN + 1];                   // make this of length maxN??
    ...
    public int delMin() { 
        ...
        exch(1, N--); 
        sink(1);
        i2t[min] = -1;            // /!\   min = t2i[1] prima che dopo la sink è = t2i[N+1]
        keys[min] = null;    // to help with garbage collection
        t2i[N+1] = -1;            // not needed
        return min; 
    }
    ...
    ***************************************************************************/
    private boolean greater(int i, int j) {
        return keys[t2i[i]].compareTo(keys[t2i[j]]) > 0;
    }
    ...
    ***************************************************************************/
    ...
    private class HeapIterator implements Iterator<Integer> {
        ...
        public HeapIterator() {
            copy = new IndexMinPQ<Key>(t2i.length - 1);
            for (int i = 1; i <= N; i++)
                copy.insert(t2i[i], keys[t2i[i]]);
        }
