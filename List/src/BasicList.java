...
public class BasicList< Item extends Comparable<Item> > {
	...
	// /!\ CREDO  BasicList è fatta con oggetti item che estendono Comparable
	// cioè che sono comparabili, per la quale è definito compareTo (CREDO)
	// ma allora non sarebbe meglio unl'interfaccia?
	...
	public static void main(String[] args) throws Exception {
		BasicList list;
		final int fattoreScala=5000;
		final int tentativi = 150;
		Random rand = new Random();
		FileWriter out = new FileWriter(args[0]);
		PrintWriter print = new PrintWriter(out);
		long start,elapsed1;
		print.println("N,List");
		for(int i=1; i<tentativi+1;i++){
			list = new BasicList();
			for(int j=0;j<i*fattoreScala;j++){
					list.insertFront(rand.nextInt());
					}
			start = System.nanoTime();
			list.search(rand.nextInt());
			elapsed1= System.nanoTime()-start;
			print.println(i*fattoreScala+","+elapsed1);
		}
		print.close();
		out.close();
